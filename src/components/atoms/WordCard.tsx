...
export function WordCard({ word, onPress }:
    ...
    return (
        <Pressable style={style.mainBox} onPress={() => onPress(word.id)}>
            ...
            {word.definitions && word.definitions.length > 0 && < View style={style.definitionAndExample}>
                <Typography variant='body'>{word.definitions[0].definition}</Typography>
                <Typography variant='example'>{word.definitions[0].example}</Typography>
            </View>
            }
        </Pressable >
    )
